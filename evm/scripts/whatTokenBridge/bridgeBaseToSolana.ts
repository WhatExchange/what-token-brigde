...
const solanaWormholeBridgeId = process.env.SOLANA_WORMHOLE_BRIDGE_ID!;
...
async function bridgeFromSepoliaToSolana(amount: number, recipient: string) {
  ...
  const allowance = await sepoliaWhatToken.allowance(
    sepoliaWalet.address,
  ...
  if (Number(allowance) < amount) {
    await sepoliaWhatToken.approve(
      sepoliaWhatTokenBridge.address,
      100_000 * 10 ** 6
    );
  }
  ...
  if (receipt) {
    ...
    if (vaa) {
      ...
      const claimAddress = derivePostedVaaKey(
        solanaWormholeBridgeId,
        parsedVaa.hash
      );
      const claimInfo = await solanaConnection.getAccountInfo(claimAddress);
      ...
      const receivedData = await whatTokenBridgeProgram.getReceivedData(
        parsedVaa
      );
