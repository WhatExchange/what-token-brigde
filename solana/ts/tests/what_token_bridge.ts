...
describe("What token bridge", function () {
  ...
  async function instantiate() {
    ...
    [user, recipient] = await Promise.all([createUserWithLamports(connection, 10), createUserWithLamports(connection, 10)]);
  ...
  it("should initialize", async function () {
    ...
    const tx = await whatTokenBridgeProgram.initialize(wallet.publicKey, fee, whatMint);
  ...
  it("throw error if not owner", async function () {
    ...
    try {
      const updateConfigTx = await whatTokenBridgeProgram.updateConfig(user.publicKey, new BN(newFee), null, null);
      await sendAndConfirmTransaction(connection, updateConfigTx, [user]);
    } catch (_err: any) {
      const msg = 'Program log: AnchorError caused by account: config_account. Error Code: Unauthorized. Error Number: 6000. Error Message: Unauthorized.'
  ...
  it("should update config", async function () {
    ...
    const updateConfigTx = await whatTokenBridgeProgram.updateConfig(wallet.publicKey, new BN(newFee), null, null);
  ...
  it("should register foreign chain and address", async function () {
    const registerChainTx = await whatTokenBridgeProgram.registerEmitter(wallet.payer.publicKey, realForeignEmitterChain, realForeignEmitterAddress);
    await sendAndConfirmTransaction(connection, registerChainTx, [wallet.payer]);
    const emitterData = await whatTokenBridgeProgram.getForeignEmitterData(realForeignEmitterChain);
  ...
  it("should update config with enabled whitelist = true", async function () {
    const updateConfigTx = await whatTokenBridgeProgram.updateConfig(wallet.publicKey, null, null, true);
    await sendAndConfirmTransaction(connection, updateConfigTx, [wallet.payer]);
  ...
  it("should redeem what token with valid vaa", async function () {
    ...
    const signedMsg = publishAndSign(recipient.publicKey, BigInt(redeemAmount), realEmitter);

    await postSignedMsgAsVaaOnSolana(connection, signedMsg, PAYER_KEYPAIR);

    const redeemTx = await whatTokenBridgeProgram.redeemWhat(recipient.publicKey, signedMsg);
